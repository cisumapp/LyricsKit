...
public actor LRCLIBClient {
    ...
    private func request<T: Decodable>(_ endpoint: Endpoint) async throws -> T {
        ...
        for attempt in 0 ... maxRetries {
            if attempt > 0 {
                ...
                LyricsLog.debug("LRCLIB: Retrying request (attempt \(attempt)/\(maxRetries)) after \(Double(delay) / 1_000_000_000)s...")
    ...
    private func performRequest<T: Decodable>(_ endpoint: Endpoint) async throws -> T {
        ...
        if let url = request.url {
            LyricsLog.debug("Sending request to LRCLIB: \(url.path)")
        }
        ...
        LyricsLog.debug("LRCLIB response status: \(httpResponse.statusCode)")

        switch httpResponse.statusCode {
        case 200 ... 299:
            do {
                let decoded = try decoder.decode(T.self, from: data)
                LyricsLog.debug("LRCLIB request succeeded (\(data.count) bytes)")
                return decoded
            } catch let decodingError as DecodingError {
                LyricsLog.debug("LRCLIB decoding failed: \(decodingError.localizedDescription)")
                throw LyricsKitError.decodingFailed(message: String(describing: decodingError))
            } catch {
                LyricsLog.debug("LRCLIB decoding failed: \(error.localizedDescription)")
                throw LyricsKitError.decodingFailed(message: error.localizedDescription)
            }
        case 404:
            LyricsLog.debug("LRCLIB resource not found")
        ...
        case 429:
            ...
            LyricsLog.debug("LRCLIB rate limited, retry after: \(retryAfter ?? 0)s")
        ...
        default:
            LyricsLog.debug("LRCLIB HTTP error: \(httpResponse.statusCode)")
